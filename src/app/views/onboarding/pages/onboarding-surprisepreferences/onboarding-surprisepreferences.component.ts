...
export class OnboardingSurprisePreferencesComponent implements OnInit {
   ...
   setPagerNum() {
      this.surpriseSlides.getActiveIndex().then((index) => {
         this.page = ++index;
      });
   }
   ...
   async surprisePreferenceStartPopup() {
      const modal = await this.modalController.create({
         ...
         componentProps: {
            title: "Almost done!",
            description: [
               "We’re now going to show you just a few more recipes to calibrate our algorithm based on what you’ve said so far.",
               "Then you’ll be ready to make a meal plan!",
            ],
         },
      });
   ...
   private savePreferences() {
      ...
      this.dataService.postSurpriseMealRatingsToServer(this.surprisePreferenceResponse).subscribe(
         (res) => {
            this.isLoading = false;
