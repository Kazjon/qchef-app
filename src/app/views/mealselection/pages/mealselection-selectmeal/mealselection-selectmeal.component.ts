...
export class MealSelectionSelectMealComponent implements OnInit {
   ...
   private checkData(mealsPerWeek, mealSlots, recommendedMeals) {
      if (mealSlots.length <= 0) {
         ...
         this.dataService.getMealPlanSelectionFromServer(mealsPerWeek).subscribe(
            (res) => {
   ...
   private organiseMealOptions() {
      ...
      for (let i = 0; i < this.mealOptions.length; i++) {
         if (
            !this.mealOptions[i].selected ||
            (this.currentMealSlot.recipe != undefined && this.mealOptions[i].id == this.currentMealSlot.recipe.id)
         ) {
   ...
   selectMeal(mealIndex: number) {
      console.log("SELECT MEAL TRIGGERED");
