...
export class SideStoreDataService {
  ...
  markIngredientCollected(i, mealSlot) {
    const sideStoreMealsArray = this.getSideStoreMealsDataFromStorage();
    const collapsedID = this.getCollapsedID(mealSlot.recipe.id, mealSlot.id);
    const mealState = sideStoreMealsArray.find((mealState) => mealState.collapsed_id == collapsedID);

    if (mealState) {
      mealState.ingredient_checklist[i] = true;
    }

    console.log("MARK INGREDIENT COLLECTED", mealState);
  ...
  getMealSlot(mealDetails: MealSlot, isReviewed: boolean = true) {
    let collapsedID = this.getCollapsedID(mealDetails.recipe!.id, mealDetails.id);
    const sideStoreMealsArray = this.getSideStoreMealsDataFromStorage();
